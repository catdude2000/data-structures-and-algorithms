...
const count = (target, input) => {
  ...
  return input.reduce((acc, curval) =>{
    const rowCount = curval.reduce((innacc, innval)=>{
      if (innval === target) {
        return innacc + 1;
      }
      return innacc;
...
const totalSum = (input) => {
  ...
  for (let i = 0; i < input.length; i++){
    // const finSum = midSum.reduce((a, b) => a + b, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  //need to filter type typeof='number' then use mod (%)
  //need to target specific arrays use map
  //nested map eventually
  const initArray = input.filter(num => num%5===0);
  const finalArray = [];
  // for (let i = 0; i<initArray.length; i++){
  finalArray.push(Math.pow(initArray, 2));
  // }
  return finalArray;
};
